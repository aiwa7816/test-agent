...
**定义**：同一位置在系统有压/无压两种状态下的热斑强度变化。

```
操作流程：
  状态A：管路充压至0.6 MPa → 采集声像图 → 提取热斑归一化强度 p_A
  状态B：管路完全泄压至0 MPa → 采集声像图 → 提取同位置强度 p_B
...
| PDR值 | 判定 | 物理依据 |
|:---:|:---|:---|
| PDR > T₂ | **漏气候选** | 泄漏声源功率∝ΔP，有压时强度显著高于无压 |
| PDR ≈ 0 | **非漏气声源** | 电气噪声、机械噪声不响应气动压力变化 |
| PDR < 0 | 误检/伪影 | 色标缩放导致的相对变化，可剔除 |

**阈值T₂的确定**：
- 理论下限：对于0.5mm孔径@0.6MPa，泄漏声功率正比于质量流率（∝√ΔP），有压vs无压的对比度不可能为零
- 实验标定：在已知泄漏点上测量PDR，取最小泄漏（0.5mm孔径@0.6MPa）的PDR值的50%作为T₂
...
#### 判据4：空间位置约束（Spatial Constraint, SC）

**定义**：热斑位置与已知管路网络拓扑的空间关联度。

```
前置条件：建立车底管路/接头位置的先验空间地图（可通过CAD图或首次无泄漏扫描建立）

空间约束判定：
  d = 热斑质心到最近管路/接头的距离

判定规则：
  d ≤ D_max（如50mm） → 空间约束满足
  d > D_max → 大概率非管路泄漏（可能是其他超声源）
```

**对于地铁车底特定场景**：
- 制动管路软管连接部位是高概率泄漏区域
- 空气管路的法兰/快接头是次高概率区域
- 远离管路的热斑（如电气箱附近）大概率为非泄漏声源
...
| 特征 | 泄漏亮斑 | 旁瓣伪影 | 机械/电气噪声 |
|:---|:---|:---|:---|
| 形状 | 近似圆形/椭圆 | 十字形/星形/条纹 | 不规则/弥散 |
| 圆度 | C > 0.7 | C < 0.5 | 不确定 |
| 梯度 | 中心强→边缘弱（平滑衰减） | 周期性强弱交替 | 无规律 |
| 面积 | 与泄漏孔径和距离相关 | 与阵列旁瓣结构相关 | 通常较大/弥散 |
...
| 阈值 | 确定方法 |
|:---|:---|
| T₁ (SBR阈值) | 在无泄漏条件下采集≥100帧，统计帧内SBR分布，取99%分位数（约3σ水平） |
| T₂ (PDR阈值) | 制作已知最小泄漏（0.5mm孔@0.6MPa），测量其PDR值，取50%作为阈值 |
| D_max (空间约束) | 根据声像仪角分辨率和检测距离确定；如距离1m，波束宽度5°→ D_max≈87mm |
| TP阈值 (0.8) | 参考文献经验值（如LangGas中使用n₁/k₁=0.8的时间一致性准则） |
| C阈值 (0.7) | 通过仿真或已知泄漏标定样本确定 |
...
2. 计算每个样本的特征向量：[SBR, PDR, TP, SC, MF]
...
### 10.6 为什么此判据体系解决了色标缩放问题

| 问题 | 解决方式 |
|:---|:---|
| 同色不同dB | 所有判据基于**帧内相对值**（SBR）或**跨条件差分**（PDR），不依赖绝对dB |
| 模型学错规律 | 不学习"红色=泄漏"，而是学习"帧内对比度+压差响应+时空稳定性=泄漏" |
| 跨样本不可比 | 归一化为帧内相对量后，跨样本的特征值具有可比性 |
| 阈值不确定 | 阈值通过物理标定实验确定，不依赖色标数值 |
...
1. **从"声压阈值判据"转向"多维行为特征判据"**——不再追求精确dB值，而是利用泄漏声源的物理行为特征（压力响应性、时间稳定性、空间关联性、形态特征）进行判别
2. **色标不变性**——所有特征对色标缩放具有不变性或单调性，从根本上解决了声像仪图像的跨样本比较问题
3. **物理可解释性**——每个判据都有明确的声学物理依据，而非纯粹的数据驱动黑箱
4. **层级决策结构**——从计算代价低的判据开始逐级筛选，满足实时性要求
