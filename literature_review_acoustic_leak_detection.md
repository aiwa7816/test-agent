...
**定义**：在单帧声像图中，目标亮斑区域相对于周围背景区域的对比度。

```
设声像图中：
  - ROI（亮斑区域）的归一化色标位置均值：p_signal
  - 背景区域（排除亮斑）的归一化色标位置均值：p_background
  - 背景区域标准差：σ_background

帧内信背比定义：
  SBR = (p_signal - p_background) / σ_background
```
...
**阈值设定**：
- SBR > T₁ → 存在显著声源（候选热斑）
- T₁的确定方法：在无泄漏条件下统计SBR分布，取99%分位数作为T₁
...
**阈值T₂的确定**：
- 实验标定：在已知泄漏点上测量ΔSBR，取最小目标泄漏（0.5mm孔径@0.6MPa）的ΔSBR值的50%作为T₂
- 不宜使用理论公式直接计算阈值（见下方说明）
...
**此判据是最强判据**：它利用了压缩空气泄漏的独有物理特性——声源功率对管路压力的因果依赖性。任何与管路压力无关的干扰源都会被此判据排除。

---

#### 判据3：时间持续性（Temporal Persistence, TP）

**定义**：在管路持续供压条件下，热斑在连续N帧中是否稳定存在。

```
在连续N帧（如N=10帧，对应约1-3秒）中：
  对同一空间位置计算帧内SBR的时间序列：{SBR₁, SBR₂, ..., SBR_N}

时间持续性指标：
  TP = count(SBR_i > T₁, i=1..N) / N
```

**判定规则**：
| TP值 | 判定 | 物理依据 |
|:---:|:---|:---|
| TP > 0.8 | 持续性声源（漏气高度可疑） | 泄漏是稳态过程，只要有压差就持续存在 |
| 0.3 < TP < 0.8 | 间歇性声源（需进一步确认） | 可能是阀门动作、间歇机械振动 |
| TP < 0.3 | 瞬态噪声（排除） | 碰撞、电磁脉冲等偶发事件 |
...
**注意**：此处是粗粒度的"区域相关性判断"，不等于"定位"。不需要输出泄漏的精确坐标，只需判断热斑是否与管路ROI重叠。
...
| T₂ (ΔSBR阈值) | 制作已知最小目标泄漏（0.5mm孔@0.6MPa），在有压/无压条件下分别测量SBR，计算ΔSBR，取该值的50%作为阈值下限 |
...
实验设计：
  ...
  评估协议：
    - 每种工况采集≥30次独立测量
    - 采用k-fold交叉验证（如k=5）避免过拟合
    - 报告95%置信区间

  性能目标（参考工业标准）：
    - PD ≥ 90%（漏报率 ≤ 10%）
    - PFA ≤ 5%（误报率 ≤ 5%）
    - 参考：美军ADA283775标准要求PD≥90%, FAR≤1次/24h
```
